...
- optional low-emphasis `推荐度 n · 描述性程度` / `Fit n · descriptive degree` plus one-sentence advisory reason when an AI score exists
...
Recommendation scores are advisory metadata, not state or a primary action. Settings expose one compact topic entry that opens a dedicated picker: interested and uninterested groups draw from the same neutral learning, academic-discipline, and research-skill presets, shown as 40–44 px pill-shaped bubbles. The user decides whether each topic should become a positive or negative signal. Each group has a “New batch” control that rotates only unselected candidates; selected bubbles remain visible and selected across refreshes. Custom input stays hidden behind an add bubble and custom values become removable bubbles after entry. Saving updates the same versioned personal profile shown by the adjacent “Personal preference profile” row; it automatically enables that profile, preserves Agent-derived signals, and does not activate an independent keyword scorer. Empty topic groups preserve all non-manual profile signals. The profile row reports state/version and offers enable/disable and view controls. A scored card shows `推荐度 n · 描述性程度`, its one-line reason, and compact human-readable labels for matched approved profile signals, including manually selected topics. The presentation-only degree is deterministic: `0–39` 不太推荐, `40–59` 一般, `60–79` 值得关注, `80–100` 强烈推荐. It is never stored as a second AI judgment. Cards without a valid AI score show a friendly unscored notice while retaining their core card content. Never use recommendation color, ordering, or automation to override the user's explicit selection decision.
...
Pure-image cards display the original image and one concise visual preview. A known multimodal model is enabled automatically, while a custom model requires an explicit “Image understanding” setting. Successful visual cards are labeled as AI recognition and may carry the same personal-profile score and matched preference names as text cards; unsupported or failed vision remains an honest local fallback. Text-heavy screenshots, diagrams, interfaces, and photographs are interpreted visually; OCR alone is insufficient.
