...
```text
iCloud~md~obsidian/AI
```

而不是桌面打开外层 `iCloud~md~obsidian`、移动端打开内层 `AI`。正确情况下，两端文件列表都会直接显示 `Raw`、`Wiki` 等根目录。
...
3. **聊天生成**：选择 OpenAI、DeepSeek、Qwen 或 Custom，填写服务地址、模型和 SecretStorage 密钥，然后测试连接。
...
- AI 标题与摘要会把有限长度的来源材料发送给用户配置的模型服务。
- 平台密码和 Cookie 不会自动发送。
