...
New Raw cards are written only under the three fixed category folders. The recognition card (`RawEvidenceGenerator.recognizeRaw`) returns `{category, title, preview, githubQueries, recommendation, recommendationIssue}` from one bounded AI call with at most one constrained repair, then a deterministic local fallback (`recognitionSource: 'local'`). A signal-bearing enabled personal profile is the only personal scoring input. Topic-picker selections are merged into that same profile as versioned manual signals before capture; they are never sent as a parallel keyword structure. The core card and recommendation are validated separately: invalid recommendation fields produce `recommendation: null` plus `recommendationIssue: 'invalid_output'` without discarding a valid title, category, or preview. If the complete text/link core is invalid, the single repair omits all recommendation fields; a second invalid core uses deterministic local fallback instead of failing capture. Optional human-readable profile-label matches are sanitized for display but never gate an otherwise valid score. Pure-image visual recognition follows the same metadata contract in one multimodal request, requests bounded JSON output (`max_tokens: 720`, `response_format: {type: 'json_object'}`), accepts either bare JSON or one complete Markdown JSON fence, and carries an explicit `ai`/`local` source marker. Unsupported vision, invalid visual JSON, and transport/provider failure map to distinct local fallback copy. Review derives its localized descriptive degree deterministically from the validated score (`0–39`, `40–59`, `60–79`, `80–100`); the degree is presentation-only and is not another AI-returned or persisted field. The GitHub name resolver (`resolveGitHubName`) returns `unique` (single exact match), `multiple` (up to three candidates for confirmation), or `none` (no URL fabricated). Existing schema-v1 Knowledge cards map to unselected/not-started until migrated and are never moved automatically.
...
For a personal preference profile, the Skill separately reads only user-authorized project summaries, writes a temporary candidate outside the Vault, validates and displays the complete candidate, and waits for explicit approval before atomically writing sibling `Preferences/preference-profile.json`. It must preserve every plugin-managed `manual-interest-*` and `manual-uninterest-*` signal exactly. The plugin never scans project repositories or Codex history. Conversely, the plugin topic picker replaces only those manual signals and preserves all Agent-derived signals and source hashes. Either side may create the initial profile.
