...
The user configures separate interested and uninterested keyword lists through a dedicated picker opened from SelfGrow settings. Both groups use the same neutral preset pool scoped to learning, academic disciplines, research methods, and scholarly skills, so no topic is intrinsically positive or negative. Preset topics appear as one-tap bubbles. Each group can request a new batch of unselected candidates without hiding or clearing any selected bubble. Custom text input remains hidden until explicitly requested, and saved custom values become removable bubbles. Save and recommendation scoring stay disabled until both groups contain at least one keyword. The same bounded AI call may then return an advisory `0–100` base score, a one-sentence reason, and exact configured keywords semantically matched by the current capture. `preference-protocol.json` contains only generic scoring rules; no personal profile ships with the plugin. Optionally, the user-reviewed `selfgrow-wiki` Skill can derive weighted content-quality signals from explicitly authorized project summaries and atomically store them in sibling `Preferences/preference-profile.json`; the plugin dynamically validates this Vault-local file, sends no source records to the model, rejects invented signal IDs, and applies approved weights to the base score. The score, generic protocol version, optional profile version, and matched keywords/signals are stored with the Raw card and shown quietly beside its preview. Missing, disabled, or invalid profiles fall back to keyword-only scoring. Changing settings or the profile affects only future captures; recommendations never auto-select, reject, delete, reorder, prioritize processing, or change Wiki eligibility.

Pure-image captures use one multimodal request when the selected model is known or explicitly marked as supporting image input. That request produces the category, concise title, visual preview, and—when both keyword groups are configured—the same recommendation metadata. If the model lacks image support or the visual call fails, SelfGrow retains the original image, writes an honest local fallback, and never labels it as AI recognition.
