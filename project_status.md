...
- Model-picker integration: `feature/model-picker-integration` starts from the latest `origin/main`, fast-forwards through the backed-up `fix/mobile-review-stability` UI/Skill work, and merges collaborator commit `cb99b11` without changing the current Collect/Review markup or `styles.css`. Settings can load available IDs from an OpenAI-compatible `/models` endpoint, retain manual model entry, and add the Kimi preset; provider secrets remain in Obsidian SecretStorage. Unrelated Android release history, release ignores, and line-ending policy from the collaborator branch were deliberately excluded. `npm run check` passes formatting, lint, all 277 tests in 35 files, source/test typechecking, and production build; the `selfgrow-wiki` self-test also passes. The build has not been installed or pushed. Real-device acceptance remains for loading/refreshing model lists, selecting an unknown/current model, provider authentication errors, and unchanged Collect/Review UI on desktop and mobile.
