...
- Mobile Review navigation refinement: `RawReviewView` is now an Obsidian navigable view, so double-tapping a preview card opens the Raw note in native history and the host Back action returns directly to Review instead of requiring the plugin to be reopened. The sticky multi-select bar is tuned another 8 px downward relative to `--view-bottom-spacing`, matching the user's corrected iPhone reference while retaining the host's dynamic navbar/safe-area offset. Formatting, lint, all 267 tests in 34 files, source/test typechecking, and production build pass; the bundle is 524,102 bytes raw and 126,713 bytes gzip. Real iPhone acceptance remains for the enabled Back action, restored Review view, batch-bar clearance above the floating navbar, and keyboard transitions. This build has not been installed into either live plugin directory.
