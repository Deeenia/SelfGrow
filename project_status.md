...
- Mobile Review navigation refinement: `RawReviewView` is now an Obsidian navigable view, so double-tapping a preview card opens the Raw note in native history and the host Back action returns directly to Review instead of requiring the plugin to be reopened. The sticky multi-select bar is tuned another 8 px downward relative to `--view-bottom-spacing`, matching the user's corrected iPhone reference while retaining the host's dynamic navbar/safe-area offset. Formatting, lint, all 267 tests in 34 files, source/test typechecking, and production build pass; the bundle is 524,102 bytes raw and 126,713 bytes gzip. The validated build is installed identically in both desktop and iPhone/iCloud plugin directories; `main.js` matches source SHA-256 `6C4674F2D6DC14499FAACEE9C0A86C24EBB137EFEEB116EABDB6AB95E99D4BBC`, `styles.css` matches `A8C9025FE6EBA270A2148A84E60F9D4A6A8A343507015A01374AB728160E265C`, and both device-specific `data.json` files were preserved byte-identically. Real iPhone acceptance remains for the enabled Back action, restored Review view, batch-bar clearance above the floating navbar, keyboard transitions, and iCloud plugin reload.
