...
- Mobile batch safe-area fix: the sticky Raw multi-select bar now adds Obsidian 1.13.7's native `--view-bottom-spacing` to its bottom offset, so the host-provided mobile navbar, iOS safe area, and keyboard adjustment remain below the SelfGrow actions; desktop and older-host behavior retain the existing spacing through a zero fallback. Formatting, lint, all 267 tests in 34 files, source/test typechecking, and production build pass; the bundle remains 524,083 bytes raw and 126,704 bytes gzip. The validated build is installed identically in both desktop and iPhone/iCloud plugin directories; `styles.css` matches source SHA-256 `333C156D699D7D1CB80271A60857C9D80D073197E08C5BBD24F7438E4AEFDC21`, and both device-specific `data.json` files were preserved byte-identically. Real iPhone acceptance remains for the visible batch bar above the floating Obsidian navbar with and without the keyboard after iCloud plugin reload.
