...
- `feature/multi-harness` integration (2026-08-23): merged `fix/mobile-review-stability` into the multi-harness branch and kept the collaborator tooling files (`.gitattributes`, `CHANGELOG.md`, release ignores, harness docs, `bootstrap` command). The Python guard conflict was resolved so `bootstrap` coexists with `link-project`, `project-status`, `validate-project-summary`, `apply-project-summary`, and `unlink-project`. The `feature/model-picker-integration` cleanup commit that deletes those tooling files was intentionally not applied because the deletions were not requested by the collaborator who owns those files. Formatting, lint, source/test typechecking, all 288 tests in 35 files, Python `py_compile`, and esbuild build pass; Python `self-test` still needs a final run in the user's PowerShell.
