...
## 0. Latest stabilization pass (2026-08-22)

- Preference-baseline presentation and visual regression completion (2026-08-25): Review now derives a stable localized degree beside every validated score (`0–39` low fit / 不太推荐, `40–59` moderate / 一般, `60–79` worth reviewing / 值得关注, `80–100` strongly recommended / 强烈推荐) without adding another model-returned or persisted field. A dedicated pure-image regression proves that empty keyword groups still produce a score, reason, human-readable profile match, and `profileVersion` from the complete approved profile while excluding internal IDs and source records. Targeted preference tests pass 32/32; the equivalent full gate passes formatting, lint, 37 test files / 307 tests, source and test typechecking, and production build at 568,379 bytes raw / 139,608 bytes gzip. Project and installed `selfgrow-wiki` self-tests pass, and the test Vault profile reports `ready`. The validated build is installed in the test Vault: `main.js` SHA-256 `F0E7D46CADA82444A3FF9BE0B4739A2990D1D6B22E8894595118D93C993DAC12`, `styles.css` `D7A9E3CE78438C4F99CE49466AB50B8297F3EEC3160084E8197B66785E753980`, and `manifest.json` `8150442D1DE96AF033103CEF9ABFDD2C796C131FCA6B7FB3F99E1E96E93AF216`; `data.json` remained byte-identical.
- Natural-language preference scoring repair (2026-08-25): text/link and pure-image recognition now validate title/category/preview separately from recommendation fields, so a missing or invalid score records a friendly unscored status without failing the capture. The model receives the complete reviewed positive/negative preference labels, descriptions, and weights without internal signal IDs or source records, directly returns the final score and reason, and may optionally report human-readable matched preference names. Either keyword group or an enabled Vault-local profile can independently enable scoring; keywords are no longer required to use a profile. `profileVersion` remains attached to newly scored cards, and historical cards are never silently rewritten.
- User-keyword, personal-profile, and multimodal-card refinement (2026-08-23): recommendation scoring requires user-editable interested and uninterested keywords from one neutral learning/academic/research pool; “换一批” rotates only unselected candidates, and selected bubbles remain visible. An optional Vault-local `Preferences/preference-profile.json` adds deeper weighted signals without bundling personal data. The `selfgrow-wiki` Skill reads only user-authorized project summaries, validates a complete candidate outside the Vault, and atomically writes it only after explicit approval. The plugin dynamically validates the profile for both text and visual captures, never sends its source records to the model, rejects invented signal IDs, applies approved weights to the model's base keyword score, persists generic/profile versions and match labels, and falls back to keyword-only scoring when the profile is missing, disabled, or invalid. Pure-image captures retain the same honest multimodal/local-fallback boundary; model-list failures never fake a loaded catalog. Formatting, lint, source/test typechecking, Skill compilation/self-test/official validation, and all 294 tests in 36 files pass; the production bundle is 567,452 bytes raw and 139,015 bytes gzip. The validated build is installed in `D:\ai_test\test_remote\test_remote\.obsidian\plugins\selfgrow\`: `main.js` SHA-256 `9A63A99DADB13FB918230C13E7D79FA00EB3C8149CCDA4571447ACE2C6FA12BF`, `styles.css` SHA-256 `7CA08552109DD27F2C65E56354F1D9FCE07F9BC0ADD38706D0A36751E6B7F537`, and the existing `data.json` remains byte-identical at `A58634CF2DB5B6C2CEAA1FA65F95533D267787F0F8F9A89B81C154F65CE10B66`.
- `feature/multi-harness` integration (2026-08-23): merged `fix/mobile-review-stability` into the multi-harness branch and kept the collaborator tooling files (`.gitattributes`, `CHANGELOG.md`, release ignores, harness docs, `bootstrap` command). The Python guard conflict was resolved so `bootstrap` coexists with `link-project`, `project-status`, `validate-project-summary`, `apply-project-summary`, and `unlink-project`. The `feature/model-picker-integration` cleanup commit that deletes those tooling files was intentionally not applied because the deletions were not requested by the collaborator who owns those files. Formatting, lint, source/test typechecking, all 288 tests in 35 files, Python `py_compile`, and esbuild build pass; Python `self-test` passed in the user's PowerShell.
- Repository Skill integration: the project-owned `selfgrow-wiki` now matches the user's installed combined Skill instead of keeping preference onboarding and workspace review on separate branches. It preserves the user-reviewed preference protocol workflow and adds explicit one-workspace/one-Raw association, read-only status, approved unlinking, exact project-summary preview, approved append under `我的笔记` / `My Notes`, hash refresh, stale-distillation reconfirmation, local-link cleanup, and handled rollback. Official Skill validation and the Skill self-test pass. The complete plugin gate passes formatting, lint, all 270 tests in 34 files, source/test typechecking, and production build; the bundle remains 530,590 bytes raw and 129,087 bytes gzip.
- Mobile review stability: Collect and Review now reuse one shared SelfGrow leaf and detach duplicate SelfGrow leaves whenever either destination opens, preventing repeated mobile taps from accumulating SelfGrow Queue tabs while leaving unrelated Obsidian tabs untouched. Mobile Collect and Review content reserve a real block-level 180px plus safe-area-inset-bottom scroll tail so the host navbar cannot cover Save, pagination, batch actions, or the final failed card. Updated Raw cards no longer show a confirmation button: swiping right calls the existing hash-checked confirmUpdate transaction and returns the card to Awaiting distillation; completed cards reject right-swipe actions. npm run check passes formatting, lint, all 270 tests in 34 files, source/test typechecking, and production build; the bundle is 530,590 bytes raw and 129,087 bytes gzip. The validated build is installed only in the iPhone/iCloud AI Vault plugin directory; main.js SHA-256 is 1509CC202D8FC76FE668176305D237903A0A60D380CB90DE7CBE51E4A75E14A7, styles.css SHA-256 is 6426ECD9E3CC540C23135FF2574AF8F3BA41DF676003CB6F4A54BAD2F4ABDE2C, manifest.json SHA-256 is 8150442D1DE96AF033103CEF9ABFDD2C796C131FCA6B7FB3F99E1E96E93AF216, and the existing data.json remains byte-identical at E5F42DC5E2350E362C99D689F25A205478363D93A0C0AC413A1707A5B7A29380. The desktop plugin was not updated; the Windows iCloud copy has the new stylesheet, while server-side iPhone delivery still requires device confirmation. Real-device acceptance remains for iPhone bottom-navbar clearance, right-swipe feedback/state movement, duplicate-tab cleanup, Collect/Review switching, and unchanged desktop navigation.
- UI and preference-scoring integration: `fix/preference-ui-integration` starts from the latest `origin/main`, retains the complete `feature/ui-stabilization` behavior (material Collect/Review shell, paginated status tabs, responsive batch controls, mobile safe-area tuning, native Review navigation, centered collection progress, and contained GitHub candidates), and adds only low-emphasis advisory recommendation metadata from the user-approved `preference-protocol.json` version `2026-08-22`. Existing Raw scores and selection state remain unchanged. Formatting, lint, all 270 tests in 34 files, source/test typechecking, production build, and the `selfgrow-wiki` self-test pass; the bundle is 530,704 bytes raw and 129,095 bytes gzip. The validated artifacts are installed identically in both desktop and iPhone AI plugin directories; `main.js` matches source SHA-256 `0BAB17BDDB75E7C28C7E890259749D6B0C7FB42E0F406D62B3354746B27175FE`, `styles.css` matches `CC2B097A98CEF9ED5D556C0C7392AC4344AAADB59B686C0A0FDD624388A3920E`, and both device-specific `data.json` files were preserved byte-identically. This branch has not been pushed or merged; desktop and iPhone visual acceptance after a full restart remains.
- Mobile UI restoration follow-up: the Collect view now matches the approved reference by removing its redundant in-view `SelfGrow` brand while retaining the native `SelfGrow Queue` host title and the compact Collect/Review switch; Review retains its own `SelfGrow` brand. The restored UI keeps the bordered composer surface, inline category selector and 44 px add button, outlined file control, underline status pagination, mobile safe-area batch bar, centered collection progress, and low-emphasis preference recommendation metadata. The complete gate passes formatting, lint, all 270 tests in 34 files, source/test typechecking, and production build; the bundle is 530,644 bytes raw and 129,073 bytes gzip. Source, desktop, and iPhone AI plugin artifacts match: `main.js` SHA-256 `F8DF3FC63BC8386C32706420B90BB87415BE5C8802530B44F8ADA9DC46FC46F6` and `styles.css` SHA-256 `CC2B097A98CEF9ED5D556C0C7392AC4344AAADB59B686C0A0FDD624388A3920E`; both existing `data.json` files were preserved byte-identically. Real iPhone acceptance requires a full app restart so Obsidian unloads the previously cached preference-branch stylesheet.
...
- Mobile Review navigation refinement: `RawReviewView` is now an Obsidian navigable view, so double-tapping a preview card opens the Raw note in native history and the host Back action returns directly to Review instead of requiring the plugin to be reopened; the user confirmed the native navigation is sufficient, so no custom Back button is added. The sticky multi-select bar is tuned 24 px downward relative to `--view-bottom-spacing`, matching the user's corrected iPhone reference while retaining the host's dynamic navbar/safe-area offset. Formatting, lint, all 267 tests in 34 files, source/test typechecking, and production build pass; the bundle is 524,102 bytes raw and 126,713 bytes gzip. The user-tuned build is installed identically in both desktop and iPhone/iCloud plugin directories; `main.js` matches source SHA-256 `6C4674F2D6DC14499FAACEE9C0A86C24EBB137EFEEB116EABDB6AB95E99D4BBC`, `styles.css` matches `07740512AF6DA87912AA0130DF0066C47B5977298D7B4AB96D30E7344FD3D97E`, and both device-specific `data.json` files were preserved byte-identically. Real iPhone acceptance remains for the restored Review view, final batch-bar clearance above the floating navbar, keyboard transitions, and iCloud plugin reload.

- Mobile batch safe-area fix: the sticky Raw multi-select bar now uses Obsidian 1.13.7's native `--view-bottom-spacing` directly, so the host-provided mobile navbar, iOS safe area, and keyboard adjustment remain below the SelfGrow actions without the extra 12 px visual gap; desktop and older-host behavior retain a zero fallback. Formatting, lint, all 267 tests in 34 files, source/test typechecking, and production build pass; the bundle remains 524,083 bytes raw and 126,704 bytes gzip. The user-tuned build is installed identically in both desktop and iPhone/iCloud plugin directories; `styles.css` matches source SHA-256 `7310579BB9A75CC80FF7C8AAA85C877051B5151E7F901BFB1D34724F6379C047`, and both device-specific `data.json` files were preserved byte-identically. Real iPhone acceptance remains for the visible batch bar immediately above the floating Obsidian navbar with and without the keyboard after iCloud plugin reload.

- Review responsive-control fix: desktop status tabs no longer inherit Obsidian's boxed button shadow and now use centered spacing above 520 px. The sticky batch bar and its conditional actions now explicitly honor their `hidden` attributes despite scoped flex/button styling, so the erroneous `已勾选 0 条` bar disappears; when active on mobile, the bar stays on one horizontally contained row with compact gaps and padding so `完成` does not wrap below the other actions. All controls retain a 44 px minimum target height. Formatting, lint, all 267 tests in 34 files, source/test typechecking, and production build pass; the bundle remains 524,083 bytes raw and 126,704 bytes gzip. The validated build is installed identically in both desktop and iPhone/iCloud plugin directories; `styles.css` matches source SHA-256 `E6D6F4FAA2E6194A2B0C1B545F10AA4544A131A2C36AA867DB6B611E55EC677D`, and both device-specific `data.json` files were preserved byte-identically. Real Obsidian acceptance remains for the desktop tab spacing, mixed-selection batch actions at about 390 px, narrow-width horizontal containment, light/dark themes, and iCloud plugin reload.
...
- Agent-neutral UI copy refinement on 2026-08-22: Raw review states now say `已选择，等待智能体` / `Selected; awaiting agent` and `智能体处理中` / `Agent processing`, while the image-understanding fallback and design-system handoff guidance refer to a generic supported coding agent instead of Codex. The remaining lowercase `codex` occurrence in tests is preserved inside simulated user-pasted source content rather than plugin UI. The complete gate passes formatting, lint without warnings, all 270 tests in 34 files, source/test typechecking, and production build; the bundle is 530,682 bytes raw and 129,092 bytes gzip. This branch has not been installed, pushed, or merged.
- Agent-neutral UI copy installation on 2026-08-22: the validated `main.js`, `manifest.json`, and `styles.css` were installed identically to the desktop Vault-root and iPhone `AI` plugin directories. Source and both installations match SHA-256 `1D64E5E7D9196F2F1F8FF66FA9904A6728069D0B15CD7907AEA7DEE3FEAB5726` for `main.js`, `8150442D1DE96AF033103CEF9ABFDD2C796C131FCA6B7FB3F99E1E96E93AF216` for `manifest.json`, and `CC2B097A98CEF9ED5D556C0C7392AC4344AAADB59B686C0A0FDD624388A3920E` for `styles.css`. Both device-specific `data.json` files were preserved byte-identically. Real-device acceptance remains for a full Obsidian restart and visible Chinese/English agent-neutral status copy.
- Task-055 unified personal preference profile completed on 2026-08-26. Topic selections now revise the same versioned personal preference profile used by link and pure-image recommendation scoring instead of forming a parallel scoring input. Both supported initialization orders preserve their existing evidence: topics may create the base profile before an optional Agent project scan, or an Agent-generated profile may receive optional topic signals later without losing project-derived signals or sources. AI title, category, and preview generation survive malformed recommendation output through a constrained core-only repair and deterministic local fallback; recommendation failure remains card-local and non-blocking. Pure-image and ordinary-link capture use the same profile-only scoring contract. The complete gate passes formatting, lint without warnings, all 314 tests in 38 files, source/test typechecking, and production build; the bundle is 570,389 bytes raw and 140,080 bytes gzip. Skill compilation, isolated self-test, and official validation pass. The validated plugin was installed to the local test Vault with source/installed `main.js` SHA-256 `47E0E739F09D1D71C4A1BB007761BC174ED226859770AF7DAF13750C31642083`; `data.json` remained byte-identical. The personal Skill installation matches the project source and passes its installed self-test.
- Visual response tolerance follow-up on 2026-08-26: successful pure-image multimodal responses no longer lose their visual card merely because a provider surrounds JSON with prose, returns text content parts, uses localized/case-varied category values, or emits more than one preview sentence. The plugin extracts the first balanced JSON object, normalizes valid fields, locally retains useful natural-language visual descriptions, and uses only the first response text for one core-only repair when necessary; the original image is uploaded exactly once and a repair failure cannot replace the first result with a misleading connection error. Recommendation failure remains isolated. The complete gate passes 317 tests in 38 files and produces a 573,990-byte bundle (141,395 bytes gzip). The local test Vault installation matches source `main.js` SHA-256 `FC95425AD9306016E8B506732F648CD9CB83894DF906684A84E9EFAC3C7D67AD`; its `data.json` was preserved.
- Provider vision-state follow-up on 2026-08-26: known multimodal model IDs are normalized case-insensitively across current settings and per-secret profiles, so stale persisted `multimodal: false` values cannot disable Kimi K3 or DeepSeek V4 Flash Vision. The settings toggle is visibly enabled and locked for known visual models, while custom models retain the explicit manual switch. Kimi image requests now use `max_completion_tokens` and omit unsupported generic sampling/JSON-mode parameters; DeepSeek and Qwen retain their compatible request shapes. The complete gate passes 318 tests in 38 files and produces a 574,575-byte bundle (141,591 bytes gzip). The local test Vault installation matches source `main.js` SHA-256 `74D7A3B5EFC08C83A850A7FB06E1227497C7E8742AFFA9B8C268C89EC256E1E7`; its `data.json` was preserved.
- AI generation compatibility repair on 2026-08-27: `fix/ai-generation-compat` was created from the latest `origin/main` and fast-forwarded to the unchanged PR #6 baseline. Text and pure-image generation now accept OpenAI-compatible assistant content as a string, text-content parts, or a non-empty `reasoning_content` fallback, and text recognition extracts the first balanced JSON object when a provider surrounds it with prose. Valid title/category/preview fields still survive invalid recommendation output. Visual protocol failures distinguish a model that is not multimodal from an incompatible provider response, so the fallback no longer falsely claims the configured vision model is disabled. Targeted regression tests pass 39/39; the complete gate passes formatting, lint, 38 test files / 322 tests, source and test typechecking, and production build at 575,842 bytes raw / 141,929 bytes gzip. The validated artifacts are installed identically in the desktop Vault-root and mobile `AI` Vault plugin directories: `main.js` SHA-256 `D1C4537CA6947EEB4A6C58F8D2BE3ABCC6501592BFFB7B18718E3541D02FE556`, `manifest.json` `8150442D1DE96AF033103CEF9ABFDD2C796C131FCA6B7FB3F99E1E96E93AF216`, and `styles.css` `D7A9E3CE78438C4F99CE49466AB50B8297F3EEC3160084E8197B66785E753980`; both existing `data.json` files remained byte-identical. The branch remains local with no push or merge. Real-provider acceptance remains for one ordinary-text capture and one pure-image capture after a full plugin reload.
- AI card completion follow-up on 2026-08-27: configured Chinese generation now requires a Chinese preview and Chinese title except for a single untranslatable project/Skill brand token, rejects titles ending in conjunctions or unfinished symbols, and rejects previews without sentence-ending punctuation instead of storing visibly truncated output. Provider reasoning prose is never exposed directly as a visual card; it may only feed the existing constrained formatting repair, while complete JSON embedded in `reasoning_content` remains compatible. JSON output budgets increase from 720 to 2,048 tokens, with Kimi retaining its provider-specific request shape. When a valid card lacks a valid recommendation, text and pure-image flows make one recommendation-only repair from the validated card and the same personal preference profile; failure still leaves the card intact and does not re-upload images. The profile at `AI/Preferences/preference-profile.json` was confirmed present and shared by the desktop-root and `AI` Vault configurations; the observed warning meant invalid model scoring output, not a missing profile. Generation regression coverage includes 40 passing tests; the complete gate passes formatting, lint, 38 test files / 331 tests, source and test typechecking, and a 580,406-byte production bundle (143,431 bytes gzip). This follow-up is not installed, pushed, or merged; real-provider acceptance remains for a new Chinese text capture and a new pure-image capture. Final review additionally removes visual title/preview hard slicing and Raw title re-truncation, anchors sentence completeness checks to the end, keeps reasoning prose out of local visual recovery, and covers recommendation repair after core repair plus retained cards when scoring repair fails.
